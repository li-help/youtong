  ...
  Future<void> _load() async {
    ...
    try {
      final res = await ApiService.listOrders(
          page: 1, pageSize: 20, status: _statusValues[_tab]);
      setState(() {
    ...
    try {
      ...
      if (res['code'] == 0) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('支付成功'), behavior: SnackBarBehavior.floating));
        _load();
      } else {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(res['msg']?.toString() ?? '支付失败'),
            behavior: SnackBarBehavior.floating));
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('网络异常，支付失败'), behavior: SnackBarBehavior.floating));
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SafeArea(
        child: Column(
          children: [
            AppPageHeader(title: '我的订单', showBack: true),
            ...
            Expanded(
              child: _loading
                  ? ListView.builder(
                      ...
                      itemCount: 6,
                      itemBuilder: (_, __) =>
                          const SkeletonListTile(hasImage: false),
                    )
                  : _error
